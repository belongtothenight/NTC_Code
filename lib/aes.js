...
AES.prototype._keyExpansion = function () {
    ...
    // assume input key is 16 utf-8 characters (128 bits)
    if (this.args.key.length !== this.args.blockSize / 8) {
        throw new Error('key length must be ' + this.args.blockSize / 8 + ' bytes');
    }
    // convert key to byte array
    let utf8Encoder = new TextEncoder();
    this.args.key = utf8Encoder.encode(this.args.key);
    // rcon
    this.args.rcon = new Uint8Array([0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]);
    // expand key
    let wCount = (this.args.round + 1) * 4;
    let w = new Array(wCount);
    for (let i = 0; i < 4; i++) {
        // first 4 words are the key / 1st key
        w[i] = [this.args.key[i * 4], this.args.key[i * 4 + 1], this.args.key[i * 4 + 2], this.args.key[i * 4 + 3]];
    }

    // let temp = w[0][0];
    // let temp1 = this._rotBytes(temp);
    // let temp2 = this._subBytes(temp1);
    // let temp3 = this._xorBytes(temp2, this.args.rcon[0]);
    ...
    for (let i = 4; i < wCount; i++) {
        ...
        temp = w[i - 1];
        if (i % 4 === 0) {
            temp = this._xorBytes1(this._subBytes(this._rotBytes(temp)), this.args.rcon[i / 4]);
        }
        w[i] = this._xorBytes2(w[i - 4], temp);
    }
    this.args.w = w;

    for (let i = 0; i < w.length; i++) {
        for (let j = 0; j < w[i].length; j++) {
            w[i][j] = w[i][j].toString(16);
        }
    }
    ...
    if (this.print._keyExpansion) {
        ...
        console.log('w', this.args.w);
    }
};

AES.prototype._rotBytes = function (input) {
