...
AES.prototype._keyExpansion = function () {
    ...
    // console.log('rcon 0', this.args.rcon[0])
    // console.log('sbox[rcon[0]]', this.args.sbox[this.args.rcon[0] - 0x01])
    // console.log('sbox[0] ', this.args.sbox[0])
    // console.log('sbox[rcon[0]]', this._subBytes(this.args.rcon[0]))
    // expand key
    let wCount = (this.args.round + 1) * 4;
    let w = new Array(wCount);
    for (let i = 0; i < 4; i++) {
        w[i] = new Array([this.args.key[i * 4], this.args.key[i * 4 + 1], this.args.key[i * 4 + 2], this.args.key[i * 4 + 3]]);
    }

    let temp = w[0][0];
    let temp1 = this._rotBytes(temp);
    let temp2 = this._subBytes(temp1);
    let temp3 = temp2 ^ this.args.rcon[0];
    ...
    for (let i = 4; i < wCount; i++) {
        let temp = new Array(w[i - 1]);
        if (i % 4 === 0) {
            temp = this._subBytes(this._subBytes(temp));
            temp[0] = temp[0] ^ this.args.rcon[i / 4 - 1];
        }
        w[i] = new Array(4);
        for (let j = 0; j < 4; j++) {
